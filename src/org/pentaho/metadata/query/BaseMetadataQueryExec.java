    ...
    switch ( param.getType() ) {
      case NUMERIC:
        if ( !( paramObj.getValue() instanceof Number ) ) {
          try {
            paramObj = new Property<Double>( Double.parseDouble( paramObj.toString() ) );
      ...
      case BOOLEAN:
        if ( !( paramObj.getValue() instanceof Boolean ) ) {
          paramObj = new Property<Boolean>( Boolean.parseBoolean( paramObj.toString() ) );
        }
        break;
      case STRING:
        if ( !( paramObj.getValue() instanceof String ) ) {
          paramObj = new Property<String>( paramObj.toString() );
        }
        break;
    }
    return paramObj;
